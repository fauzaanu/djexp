# This project is a clone of the deployment guide made by Brad Traversy in his gist

# Why make this?

Django offers speedy development and a lot of features out of the box.
However, the deployment is not as easy.
Laravel has Laravel Forge, which makes deployment a breeze.
I wanted to make a similar tool for Django, and this is the first step.
Not sure if this can be made into a tool, but I will try and your most welcome to help.

# Assumptions

- We are deploying to a vps
- We are using nginx
- We are using gunicorn
- We are using postgresql
- We are using tailwindcss
- we are using django-environ
- we are using supervisor

# Structure

Brand made the project structure, that can be for a project called btre-project.
For this to be dynamic, we need to keep some things constant.

- The project name will always be called Project
- The app name will always be called App
- I understand there can be multiple apps so you can actually change the app name to whatever you want and this should not matter.
- The only thing to keep in mind is that the app name should be the same as the folder name inside the project folder. and is added to the INSTALLED_APPS in settings.py
- Like anyother django project. and you should know this if you are in the deployment stage.
- Need multiple deployments? create a new parent folder and have the same structure inside it. but the nginx and gunicorn automated scripts will not work. you will have to do it manually. (as of now)

# End Goal

End goal is to dockerize this project and make it a tool that can be used to deploy django projects with ease.
A lot of the docker solutions for django in github are not working. I will try to make this work.

# End of read me

# Here is the original readme from Brad's gist
